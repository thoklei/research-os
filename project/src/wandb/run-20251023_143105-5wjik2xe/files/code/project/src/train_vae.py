...
def main():
    ...
    with TeeLogger(log_path):
        ...
        if args.overfit_batch and config.use_class_weights:
            print("\n" + "─" * 70)
            print("[OVERFIT MODE] Computing class weights from single batch...")
            print("─" * 70)

            from models import compute_class_weights
            from data import SingleBatchDataset

            # Get the unique batch from the SingleBatchDataset
            if isinstance(train_loader.dataset, SingleBatchDataset):
                unique_batch = train_loader.dataset.batch  # Shape: (batch_size, H, W)
                print(f"  Single batch shape: {unique_batch.shape}")

                # Create a temporary dataset wrapper for the unique batch
                class TempDataset:
                ...
                temp_dataset = TempDataset(unique_batch)

                # Compute weights from just the unique batch
                ...
                print("[WARNING] Expected SingleBatchDataset, skipping class weights")
